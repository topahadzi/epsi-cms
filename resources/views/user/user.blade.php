...
<div class="col-md-12">
    <div class="card card-plain">
        <div class="header row">
            <div class="col-lg-10">
                <h4 class="title"><b>List User</b></h4>
            </div>
            <div class="col-lg-2">
    ...
    $('#table').DataTable({
        ...
        columns: [
            ...
            },
            {
                "mData": "verifikasi",
                "mRender": function (data, type, row, meta) {
                    return data ? `<i class="fas fa-check" style="color: green;"></i>` : `<p  style="color: red;">X</p>` ;
                }
            },
            {
                "mData": "_id",
                "mRender": function (data, type, row) {
                    let detail = `{{ url('/user/${data}') }}`;
                    let edit = `{{ url('/user_edit/${data}') }}`;
                    let dlt = `{{ url('/user_delete/${data}') }}`;
                    return `<button class="btn btn-info active update-data" data-id=${data} style="border-radius:0;"><i class="fas fa-check"></i></button>`+
...
$(document).on('click', '.update-data',function(e){
    ...
    $.ajax({
        url: `{{config('app.api_url')}}user/update/${id}`,
        type: 'POST',
        data: formData,
        dataType: "json",
        beforeSend: function (xhr) {
        ...
        fail: function(xhr, textStatus, errorThrown){
            swal("Gagal", "Gagal Verifikasi", "error");
        }
    });
});
